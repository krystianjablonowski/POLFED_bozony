...
1. **Resolved maxima.** The complete list is the table above. Unresolved reasons: broad_bootstrap_interval=10, local_fit_rejected=10, low_bootstrap_valid_fraction=10, low_peak_significance=10, peak_estimators_disagree=10, plateau_includes_U0=10, scan_boundary=10.
...
3. **Small W.** For the independent channel theory, the median relative error of `A W + B W^3` is 0.000264 (maximum 0.046). The ED coefficient fits and bootstrap intervals are in `ed_small_W_fits.csv`.
4. **Large W theory.** The median relative error of `C sqrt(W)` is 0.119 (maximum 0.119); `effective_exponent.pdf` shows the approach to beta=1/2.
